...
def parse_args():
    ...
    parser.add_argument(
        ...
        # COCO:  If not specified, ``['AR@100', 'AR@300',
    ...
    parser.add_argument('--eval_dataset', default='coco', type=str, 
        ) # The class split is specified in the config file
...
def main():
    ...
    if args.modality == "rgb": # Phase-II evaluation must be on RGB
    ...
    if args.eval_dataset == "coco":
        pass
    elif  args.eval_dataset == "uvo":
        cfg.data_root = 'uvo/'
        cfg.data.train.ann_file = cfg.data_root + 'UVO_frame_train.json'
        cfg.data.train.img_prefix = cfg.data_root  + 'uvo_videos_sparse_frames/'
        cfg.data.val.ann_file = cfg.data_root + 'UVO_frame_val.json'
        cfg.data.val.img_prefix = cfg.data_root  + 'uvo_videos_sparse_frames/'
        cfg.data.test.ann_file = cfg.data_root + 'UVO_frame_val.json'
        cfg.data.test.img_prefix = cfg.data_root  + 'uvo_videos_sparse_frames/'
    elif  args.eval_dataset == "ade20k":
        cfg.data_root = 'ade20k/'
        cfg.data.train.ann_file = cfg.data_root + 'annotations/train.json'
        cfg.data.train.img_prefix = cfg.data_root  + 'train/'
        cfg.data.val.ann_file = cfg.data_root + 'annotations/val.json'
        cfg.data.val.img_prefix = cfg.data_root  + 'val/'
        cfg.data.test.ann_file = cfg.data_root + 'annotations/val.json'
        cfg.data.test.img_prefix = cfg.data_root  + 'val/'
    elif  args.eval_dataset == "lvis":
        cfg.data.test.img_prefix = cfg.data_root
        if args.modality == 'rgb':
            cfg.data.test.ann_file = 'lvis/lvis_v1_val_noncoco.json'

    # dataset = build_dataset(cfg.data.test)
    if cfg.data.test.pipeline[1]['type'] == "LoadAnnotations": # In test mode, parsing in CustomDataset won't load in ann_info
